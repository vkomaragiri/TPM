...
for dataset in datasets:
    for evid_percent in evids:
        ...
        data_directory = '/Users/vasundharakomaragiri/research/TMap.12.2020/TMap/data/oracle-uai-posterior-samples/'
        ...
        for k in range(5):
            ...
            tpm_estimate = np.loadtxt(fname=data_directory+dataset+'_'+evid_percent+'_percent_'+str(k)+'.tpm2', delimiter=',', usecols=np.arange(100000))
            lw_estimate = np.loadtxt(fname=data_directory+dataset+'_'+evid_percent+'_percent_'+str(k)+'.lw', delimiter=',', usecols=np.arange(100000))
            ...
            #for nsamples in [100, 200, 300, 400, 500, 600, 700, 800, 900, 1000]:
                ...
                oracle_tpm_log_wt = np.exp(logsumexp(oracle_tpm_estimate[indices])-np.log(nsamples))
                tpm_log_wt = np.exp(logsumexp(tpm_estimate[indices])-np.log(nsamples))
                lw_log_wt = np.exp(logsumexp(lw_estimate[indices])-np.log(nsamples))
            
                #tpm_log_wt /= 5.0
                #lw_log_wt /= 5.0
                results.append([k, nsamples, oracle_tpm_log_wt, tpm_log_wt, lw_log_wt])

        np.savetxt(X = np.array(results), fname='/Users/vasundharakomaragiri/research/TMap.12.2020/TMap/results/oracle-uai-posterior-samples/'+dataset+'_'+evid_percent+'_percent.csv')
