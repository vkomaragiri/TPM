...
def write_PBS_script(path, data_dir, ds, n, dtype):
    ...
    args = data_dir+ds+' '+n+' '+samples_dir+ds+dtype+'.data'
...
def runExperiments():
    ...
    data_dir = r'/nfs/experiments/uai08-benchmarks/uai06/pe/'

    num_samples = ['64000', '16000', '20000']
    dtypes = ['.ts', '.valid', '.test']

    base_path = r'/nfs/experiments/vasundhara/TMaP/sample_uai_bn/'
    sdir = base_path+'scripts/'
    for fname in os.listdir(data_dir):
        if fname.split('.')[-1] != 'evid':
            ds = fname
