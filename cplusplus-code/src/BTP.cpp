...
BTP::BTP(CLT &clt, const unordered_map<int, int> &varid_ind_) {
    pe = 1.0;
    variables = clt.variables;
    varid_ind = varid_ind_; 
    Utils::getOrder(variables, clt.functions, order, varid_ind);
    ...
    for(auto &func: clt.functions){
        ...
        for(auto var: newf.variables){
            if(varid_ind.empty()){
                ...
                if(bucket_func > var_pos[varid_ind.at(var->id)]){
                    bucket_func = var_pos[varid_ind.at(var->id)];
                }
            }
        }
...
void BTP::upward_pass(){ //leaves to root
    if(upward_performed) return;
        ...
        for(int i = 0; i < buckets.size(); i++){
            ...
            int cl_sz = bucket_variables.size();
            if(treewidth < cl_sz){
                treewidth = cl_sz;
            }
            vector<ldouble> mult, sum;
            ...
            if(varid_ind.empty()){
                ...
                for(auto &var: newf.variables){
                    if(var_pos[varid_ind.at(var->id)] < bucket_loc){
                        bucket_loc = var_pos[varid_ind.at(var->id)];
                    }
                }
            }
